\renewcommand{\contentsname}{Indice}
\tableofcontents
% * PARA MODIFICAR ALGÚN TEMA, HAY QUE IR AL ARCHIVO CORRESPONDIENTE, Y SI SE QUIERE AÑADIR ALGÚN SUB APARTADO, SE AÑADE AHÍ, CON UNA SEPARACION DE 5 CM ENTRE CADA APARTADO *%
\newpage
\section{Introducción}
\input{./src/themes/intro}
\newpage
\section{Tema 1: Campo Electrostático}
\input{./src/themes/t1}
\newpage
\section{Tema 2: Campo Magnetostático}
\input{./src/themes/t2}
\newpage
\section{Tema 3: Circuitos de Corriente Continua}
\input{./src/themes/t3}
\newpage
\section{Tema 4: Inducción Electromagnética}
\input{./src/themes/t4}
\newpage
\section{Tema 5: Circuitos de Corriente Alterna}
\input{./src/themes/t5}
\newpage
\section{Tema 6: Ondas Electromagnéticas}
\input{./src/themes/t6}